...
export default function LandingPage() {
    ...
    const handleUpload = async () => {
        ...
        formData.append("quality", quality.toString());  // Dynamic quality value
        ...
        try {
            ...
            link.href = imageUrl;
            link.download = "resized-image.jpg"; // Default file name for download
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);

            setImageUrl(imageUrl); // Set the image URL for preview
        } catch (err) {
    ...
    return (
        <div className="container mx-auto p-6">
            ...
            <section className="bg-white shadow-lg rounded-lg p-6 mb-8">
                ...
                <label className="block mt-2">Quality (1-100):</label>
                <input type="number" value={quality} onChange={handleQualityChange} min="1" max="100" className="border p-2 rounded w-full" />
                <button 
                    onClick={handleUpload} 
                    disabled={loading} 
                    className="bg-blue-500 text-white px-6 py-2 rounded mt-4 disabled:opacity-50">
